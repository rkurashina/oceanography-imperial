...
c Dimension sizes
      ...
      parameter(nlvls = 3,spatial_dims = 2,ndims = 6)
...
C ---------------------------------------------------------------------------------
     ...
     & ,x1,y1
     & ,x2,y2,x3,y3,x1_coord,y1_coord,x2_coord,y2_coord
     & ,x3_coord,y3_coord
     & ,save_time,step_time)
...
c write to a single bin for a single release for npoints particles at time save_time
     ...
     & ,x2(npoints),y2(npoints),x3(npoints),y3(npoints)
      integer, intent(in) :: x1_coord(npoints),y1_coord(npoints)
     & ,x2_coord(npoints),y2_coord(npoints),x3_coord(npoints)
     & ,y3_coord(npoints)
...
c Variable names
      ...
      real*8 traj_temp(2,npoints,3)        !3 here is nlvls, 2 is spatial_dims, both below
      integer coor(2,npoints,3),start_info(2)
...
c Write x, y to the temporary trajectory variable
      do j = 1,npoints
              ...
              traj_temp(1,j,2) = x2(j)
              traj_temp(2,j,2) = y2(j)
              traj_temp(1,j,3) = x3(j)
              traj_temp(2,j,3) = y3(j)
              coor(1,j,1) = x1_coord(j)
              coor(2,j,1) = y1_coord(j)
              coor(1,j,2) = x2_coord(j)
              coor(2,j,2) = y2_coord(j)
              coor(1,j,3) = x3_coord(j)
              coor(2,j,3) = y3_coord(j)
      end do
...
C --------------------------------------------------------------------------------
     ...
     & ,bin_boundaries,t_len,time)
      ...
      real*8 time(nrel,t_len),bin_boundaries(nbins,nrel)

      character*(*) t_name,rel_name,bin_name
      parameter(t_name = 'Time')
      parameter(rel_name = 'Release Number')
      parameter(bin_name = 'Bin Boundaries')
      ...
      retval = nf_get_vara_double(ncid,bin_varid
     & ,start,count,bin_boundaries)
      if (retval .ne. nf_noerr) call handle_err(retval)
      print*,'bin boundaries read'
