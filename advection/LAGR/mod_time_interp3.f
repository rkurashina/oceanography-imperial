...
c RK4 time integration
     ...
     &                    (time_cubic(k)-time_cubic(i))
            ...
            subroutine read_psi(file_name,ii,jj,time_interp
     & ,psi1,psi2,psi3)
            ...
            real psi1_s(ii,jj,4), psi2_s(ii,jj,4), psi3_s(ii,jj,4)
            real*8 psi1(ii,jj,4), psi2(ii,jj,4), psi3(ii,jj,4)
            
            character*(*) file_name

            integer retval, ncid, nlvls, ndims
            parameter(ndims=4,nlvls=3)
            ...
            retval = nf_get_vara_real(ncid,psi_varid,start,count,psi1_s)
            if (retval .ne. nf_noerr) call handle_err(retval)

            start(3) = 2
            retval = nf_get_vara_real(ncid,psi_varid,start,count,psi2_s)
            if (retval .ne. nf_noerr) call handle_err(retval)

            start(3) = 3
            retval = nf_get_vara_real(ncid,psi_varid,start,count,psi3_s)
            if (retval .ne. nf_noerr) call handle_err(retval)
