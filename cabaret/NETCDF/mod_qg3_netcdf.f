      module MOD_qg3_NETCDF

      use mod_netcdf_error

      contains

c-------------------------NEED TO CHANGE FOR 3 ENERGIES---------------------------
...
C ----------------------- AND ENERGIES AND TIME ---------------------------------------------
     ...
     +         Time_UNITS)
        ...
        retval = nf_def_var(ncid, Psi_NAME, NF_FLOAT, NDIMS, dimids,   !! Creating Main varialbes
     +        psi_varid)
        if (retval .ne. nf_noerr) call handle_err(retval)
...
C ---------------------------------------------------------------------
       ...
       subroutine write_netcdf(FILE_NAME,psi1,psi2,psi3,
     +    epot,ekin1,ekin2,Num_x,Num_y,save_tim,step_tim)
        ...
        integer Num_x, Num_y, step_tim
        character*(*) FILE_NAME
        real*8 epot, ekin1, ekin2, save_tim
        real psi1(Num_x,Num_y), psi2(Num_x,Num_y), psi3(Num_x,Num_y)        

        integer NDIMS, NLVLS, ETYPE, i, j
        parameter (NDIMS = 4, ETYPE = 4, NLVLS=3)
        integer ncid, retval, dimids(NDIMS), dimid(2),
     +   x_dimid, y_dimid, rec_dimid, lvl_dimid, Et_dimid,
     +   start(NDIMS), count(NDIMS), start1(2), count1(2)
        ...
        real tp_psi(Num_x,Num_y,NLVLS)    !! Define temporary variables
        real *8 tp_ene(ETYPE)
        ...
        retval = nf_put_vara_real(ncid, psi_varid, start, count,      !! Update Data
     +        tp_psi)
        if (retval .ne. nf_noerr) call handle_err(retval)
